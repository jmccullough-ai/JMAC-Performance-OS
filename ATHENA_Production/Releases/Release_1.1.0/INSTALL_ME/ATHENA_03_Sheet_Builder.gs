...
function ATHENA_buildFoundationSheets_() {
  ...
  ATHENA_buildCoachPhilosophySheet_(ss.getSheetByName(ATHENA_PRODUCTION.sheets.coachPhilosophy));
  ATHENA_buildCoachDnaSheet_(ss.getSheetByName(ATHENA_PRODUCTION.sheets.coachDna));
  ATHENA_buildMovementFamiliesSheet_(ss.getSheetByName(ATHENA_PRODUCTION.sheets.movementFamilies));
...
function ATHENA_buildHomeSheet_(sheet) {
  ...
  ATHENA_title_(sheet, 'JMAC Performance OS', 'ATHENA Production — Release 1.1.0 Verified Baseline');
  ...
  const controls = [
    ...
    ['Status', 'Release 1.1.0 Installed', 'Foundation only. Workout generation begins later.']
  ];
...
function ATHENA_buildCoachPhilosophySheet_(sheet) {
  ATHENA_baseFormat_(sheet);
  const headers = ['Principle_ID', 'Principle', 'Description', 'Training_Implication', 'Active'];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  ATHENA_styleHeader_(sheet.getRange(1, 1, 1, headers.length));
  sheet.setFrozenRows(1);
...
function ATHENA_buildCoachDnaSheet_(sheet) {
  ATHENA_baseFormat_(sheet);
  const headers = ['Rule_ID', 'Rule_Category', 'Rule_Name', 'If_Condition', 'Then_Action', 'Priority', 'Applies_To', 'Active'];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  ATHENA_styleHeader_(sheet.getRange(1, 1, 1, headers.length));
  sheet.setFrozenRows(1);
...
function ATHENA_buildMovementFamiliesSheet_(sheet) {
  ATHENA_baseFormat_(sheet);
  const headers = ['Family_ID', 'Movement_Family', 'Primary_Quality', 'Progression_Level_1', 'Progression_Level_2', 'Progression_Level_3', 'Progression_Level_4', 'Progression_Level_5', 'Progression_Level_6', 'Notes', 'Active'];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  ATHENA_styleHeader_(sheet.getRange(1, 1, 1, headers.length));
  sheet.setFrozenRows(1);
...
function ATHENA_buildHealthCheckSheet_(sheet) {
  ATHENA_baseFormat_(sheet);
  ATHENA_title_(sheet, 'ATHENA Production Health Check', 'Release 1.1.0 Validation');
