...
function ATHENA_runHealthCheck() {
  ...
  const pass = results.filter(function(row) {
    return row[1] === 'PASS';
  }).length;

  const fail = results.filter(function(row) {
    return row[1] === 'FAIL';
  }).length;
